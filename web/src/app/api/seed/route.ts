 ...
 *  SkyJet (the live demo agent) + two other companies with mixed records.
 * In sim mode this resets state. In chain mode it only registers the demo company if needed.
 */
export async function POST() {
  try {
    if (backend() === "sim") {
      ...
      // an attempted out-of-envelope promise, blocked
    ...
    // --- SkyJet: the live demo company (both backends)
    const co = await kept.registerDemoCompany("SkyJet Airlines", SKYJET_ENVELOPE, 25000);
