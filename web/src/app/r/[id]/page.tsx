...
export default function ReceiptPage({ params }: { params: { id: string } }) {
  ...
  const [me, setMe] = useState<{ user?: string; backend?: string; immediateClaims?: boolean }>({});
  useEffect(() => { fetch("/api/backend").then((x) => x.json()).then(setMe).catch(() => {}); }, []);
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="card-mint rise p-6">
        ...
        <dl className="mt-5 grid gap-x-8 gap-y-3 text-sm sm:grid-cols-2">
          ...
          <div><dt className="k">Promised to</dt><dd className="mono mt-0.5">{short(r.user)} {mine && <span className="pill ml-1 bg-mint/15 text-mint">YOU</span>}</dd></div>
          ...
          <div className="sm:col-span-2"><dt className="k">Anchored</dt><dd className="mono mt-0.5 break-all text-white/60">{onChain ? <>GenLayer {me.backend === "chain" ? "" : ""}{r.tx ? <a className="underline" href={`https://genlayer-explorer.vercel.app/tx/${r.tx}`} target="_blank" rel="noreferrer">{r.tx}</a> : "(read from contract)"}</> : "simulator twin of the contract (no chain)"}</dd></div>
        </dl>
      </div>
        ...
        <div className="card p-5">
          ...
          {tab === "customer" ? (
            <div className="space-y-3">
              ...
              <p className="text-sm text-white/60">GenLayer validators read the transcript and your evidence, then rule. If upheld, <b className="text-white">${Math.max(r.amount, 0)}</b> is paid from {r.company_name}&apos;s bond automatically. Claims open on the due date{onChain && me.immediateClaims ? " — for this demo the due date is set to today so you don't have to wait five business days" : ""}.</p>
              {!mine && <p className="rounded-lg bg-amber/10 p-2 text-xs text-amber">This promise was made to a different visitor. The contract only lets the promised user claim — go make your own promise in the demo chat.</p>}
              ...
              <div className="flex flex-wrap items-center gap-3">
                {!onChain && <label className="text-xs text-white/50">Demo clock <input className="input ml-2 inline-block w-40 !py-1.5" placeholder="YYYY-MM-DD (fast-forward)" value={today} onChange={(e) => setToday(e.target.value)} /></label>}
                {!onChain && !overdue && !today && <button className="text-xs text-mint underline" onClick={() => { const d = new Date(r.due + "T00:00:00Z"); d.setUTCDate(d.getUTCDate() + 5); setToday(d.toISOString().slice(0, 10)); }}>fast-forward past due date</button>}
                <button className="btn-primary ml-auto" onClick={claim} disabled={!!busy || !evidence.trim() || !mine}>{busy === "claim" ? (onChain ? "Validators ruling on GenLayer… ~20s" : "Validators ruling…") : "File claim"}</button>
              </div>
            </div>
          ) : (
            <div className="space-y-3">
